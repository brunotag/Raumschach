...
int eval()
{
	...
	for (i = 0; i < 125; ++i) {
		if (color[i] == EMPTY)
			continue;
		if (piece[i] == PAWN) {
			pawn_mat[color[i]] += piece_value[PAWN]; // == valore base di tutti i pedoni sulla scacchiera
			...
			piece_mat[color[i]] += piece_value[piece[i]];  // == a valore base di tutti i pezzi sulla scacchiera
	}
		...
		if (color[i] == LIGHT) {
			switch (piece[i]) {
				...
				case KNIGHT:
					...
					break;
				case BISHOP:
					score[LIGHT] += bishop_pcsq[i]; //a seconda dei pezzi aggiungo il valore a seconda della posiz.
					break;
				case ROOK:
					//if (pawn_rank[LIGHT][COL(i) + 1] == 0) {  //colonna aperta o semiaperta
					...
					//}
					//if (ROW(i) == 1)
					//	score[LIGHT] += ROOK_ON_SEVENTH_BONUS; //bonus per torre sulla settima riga
					break;
				case KING:
					if (piece_mat[DARK] <= 1200)
						score[LIGHT] += king_endgame_pcsq[i];  //eval particolare x re in endgame
					else
						score[LIGHT] += eval_light_king(i);  //eval normale x il cavallo bianco
					break;
			}
		}
		else {
			switch (piece[i]) { //IDEM ma x il nero
				case PAWN:
